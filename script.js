document.addEventListener("DOMContentLoaded", () => {
  ...
  // ==========================================
  // LOGIKA ADMIN / OWNER MODAL (VERIFIKASI SUPABASE)
  // ==========================================
  ...
  // VERIFIKASI KE SUPABASE (TIDAK ADA PIN DI JS MAUPUN INSPECT ELEMENT)
  if (submitPinBtn) {
    submitPinBtn.addEventListener("click", async () => {
      const inputPasscode = adminPinInput ? adminPinInput.value : "";
      ...
      submitPinBtn.textContent = "Mengecek...";

      // Cek apakah sandi yang diinput ada di tabel admin_config
      const { data, error } = await supabase
        .from('admin_config')
        .select('*')
        .eq('passcode', inputPasscode);

      submitPinBtn.textContent = "Masuk";
